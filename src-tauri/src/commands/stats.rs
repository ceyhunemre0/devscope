...
pub async fn get_stats(state: State<'_, AppState>, range_days: u32) -> AppResult<StatsData> {
    ...
    let projects: Vec<(String,)> =
        sqlx::query_as("SELECT path FROM projects WHERE state = 'active'")
            .fetch_all(&state.pool)
            .await?;

    let mut totals: std::collections::BTreeMap<chrono::NaiveDate, u32> =
...
pub async fn list_recent_commits(
    ...
    let projects: Vec<(i64, String, String)> = match project_id {
        Some(id) => {
            sqlx::query_as("SELECT id, name, path FROM projects WHERE id = ?")
                .bind(id)
                .fetch_all(&state.pool)
                .await?
        }
        None => {
            sqlx::query_as("SELECT id, name, path FROM projects WHERE state = 'active'")
                .fetch_all(&state.pool)
                .await?
        }
    };

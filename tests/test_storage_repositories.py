...
def test_event_repo_dedupes_by_source_external_id(db_session, now):
    ...
    e1 = event_repo.upsert(
        project_id=project.id, source="git_local", type="commit",
        external_id="abc", payload={"msg": "first"}, occurred_at=now,
    )
    e2 = event_repo.upsert(
        project_id=project.id, source="git_local", type="commit",
        external_id="abc", payload={"msg": "ignored"}, occurred_at=now,
    )
    db_session.commit()
    assert e1.id == e2.id
    assert event_repo.list_since(project_id=project.id,
                                 since=now - timedelta(hours=1))[0].payload["msg"] == "first"


def test_llm_call_repo_records_and_sums_costs(db_session, now):
    repo = LLMCallRepo(db_session)
    repo.record(provider="ollama", model="llama3.1:8b", purpose="standup",
                prompt_tokens=10, output_tokens=20, cost_usd=0.001,
                duration_ms=100, succeeded=True, error=None, called_at=now)
    repo.record(provider="ollama", model="llama3.1:8b", purpose="standup",
                prompt_tokens=10, output_tokens=20, cost_usd=0.002,
                duration_ms=100, succeeded=True, error=None, called_at=now)
    db_session.commit()
...
def test_report_repo_saves_and_lists(db_session, now):
    ...
    r = repo.save(project_id=None, type="standup", content="# hi",

...
class StubRouter:
    ...
    async def complete(
        self, *, prompt: str, purpose: str, max_tokens: int = 1500, temperature: float = 0.2
    ) -> LLMResponse:
        self.last_prompt = prompt
        assert purpose == "standup"
        return LLMResponse(text=self.text, prompt_tokens=10, output_tokens=20, cost_usd=0.0)
...
async def test_standup_calls_router_and_includes_event_messages():
    ...
    events = [
        Event(
            ...
            type="commit",
            external_id="a" * 40,
            payload={"message_summary": "feat: A", "author_name": "x", "files_changed": ["a.py"]},
            occurred_at=now,
        ),
        Event(
            source="git_local",
            type="commit",
            external_id="b" * 40,
            payload={"message_summary": "fix: B", "author_name": "x", "files_changed": ["b.py"]},
            occurred_at=now,
        ),
    ]
    out = await gen.run(
        events_by_project={"my-saas": events}, since=now - timedelta(hours=24), until=now
    )

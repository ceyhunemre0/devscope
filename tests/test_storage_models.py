...
def test_event_unique_external_id_per_source():
    ...
    s = _session()
    s.add(Project(name="p", path="/tmp/p", state="active",
                  created_at=datetime.now(timezone.utc),
                  updated_at=datetime.now(timezone.utc)))
    s.commit()
    pid = s.query(Project).one().id
    now = datetime.now(timezone.utc)
    s.add(Event(project_id=pid, source="git_local", type="commit",
                external_id="abc123", payload={"msg": "x"},
                occurred_at=now, collected_at=now))
    s.commit()
    s.add(Event(project_id=pid, source="git_local", type="commit",
                external_id="abc123", payload={"msg": "y"},
                occurred_at=now, collected_at=now))
...
def test_llm_call_records_token_usage():
    s = _session()
    now = datetime.now(timezone.utc)
    s.add(LLMCall(provider="ollama", model="llama3.1:8b", purpose="standup",
                  prompt_tokens=120, output_tokens=80, cost_usd=0.0,
                  duration_ms=500, succeeded=True, called_at=now))
    s.commit()
...
def test_idea_default_state_is_suggested():
    ...
    s.commit()
    pid = s.query(Project).one().id
    s.add(Idea(project_id=pid, category="feature", title="t", description="d",
               created_at=datetime.now(timezone.utc)))
    s.commit()
